...
@section('page-title', 'Nueva Incidencia')
@section('page-subtitle', 'Complete los campos para registrar la incidencia')

@section('topbar-actions')
<span class="text-xs font-mono text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg">INC-{{ now()->year }}-XXXX</span>
@endsection

@section('content')
<form method="POST" action="{{ route('incidencias.store') }}" enctype="multipart/form-data" id="form-incidencia">
@csrf
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <select id="nf-region"
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-500">
                            <option value="">Seleccionar...</option>
                            @foreach(['Región Metropolitana','Valparaíso','Biobío','La Araucanía','Los Lagos','Antofagasta','Coquimbo','O\'Higgins','Maule','Ñuble','Los Ríos','Arica y Parinacota','Tarapacá','Atacama','Aysén','Magallanes'] as $r)
                            <option>{{ $r }}</option>
                            @endforeach
                        </select>
                    </div>
