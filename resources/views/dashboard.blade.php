...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        ...
        <div class="text-3xl font-bold text-gray-900">{{ $totalRecursos }}</div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        ...
        <div class="text-3xl font-bold text-gray-900">{{ $totalHerramientas }}</div>
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-4">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="{{ route('clientes.index') }}" class="text-sm text-red-600 font-semibold hover:text-red-700">
            Ver todos →
        </a>
    </div>
    <div class="divide-y divide-gray-50">
        ...
        <div class="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 transition-colors">
            <div class="text-xl">{{ $recurso->icono ?? '📄' }}</div>
            <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-gray-900 truncate">{{ $recurso->nombre }}</div>
                <div class="text-xs text-gray-400 font-mono">
                    {{ $recurso->cliente->nombre ?? 'Global' }} · {{ $recurso->tipo }} · {{ $recurso->tamanio }}
                </div>
            </div>
            <span class="text-xs font-mono font-bold px-2 py-1 rounded
                ...
                {{ $recurso->tipo === 'LINK' ? 'bg-gray-50 text-gray-600'    : '' }}">
                {{ $recurso->tipo }}
            </span>
        </div>
        @empty
        <div class="px-5 py-8 text-center text-gray-400 text-sm">
            ...
            <a href="{{ route('clientes.index') }}" class="text-red-600 font-semibold">Ver clientes →</a>
        </div>
        @endforelse
    </div>
</div>

{{-- Fila inferior: Últimas visitas + Avisos --}}
        ...
        @if($ultimasVisitas->isEmpty())
            <div class="p-8 text-center">
                ...
                <a href="{{ route('clientes.index') }}"
                   class="inline-block mt-3 text-xs text-red-600 font-semibold hover:text-red-700">
                    Explorar clientes →
                </a>
            </div>
        @else
            <div class="divide-y divide-gray-50">
                ...
                {{-- Armamos la URL con el parámetro reanudar solo si ultima_pagina > 1 --}}
                @php
                ...
                <a href="{{ $url }}"
                   class="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors group">
                    ...
                    <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0
                        {{ $visita->tipo === 'documento' ? 'bg-red-50' : 'bg-blue-50' }}">
                        <span class="text-lg">
                            {{ $visita->tipo === 'documento' ? '📄' : '🖥️' }}
                        </span>
                    </div>

                    {{-- Info --}}
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-medium text-gray-900 truncate group-hover:text-red-600 transition-colors">
                            {{ $visita->recurso_nombre }}
                        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden"
        ...
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <button @click="abierto = true"
                    class="text-xs bg-red-600 text-white font-semibold px-3 py-1.5 rounded-lg hover:bg-red-700 transition-colors">
                + Nuevo
            </button>
            @endrole
        </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-3 mt-1.5 flex-wrap">
                                @if($aviso->url)
                                <a href="{{ $aviso->url }}"
                                   class="text-xs text-red-600 font-semibold hover:text-red-700 transition-colors">
                                    {{ $aviso->url_texto ?? 'Ver recurso' }} →
                                </a>
                                @endif
                                <span class="text-xs text-gray-400 font-mono">
                                    {{ $aviso->publicado_at?->diffForHumans() }}
                                </span>
                                <span class="text-xs text-gray-300 font-mono">
                                    por {{ $aviso->user->name ?? 'Admin' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{ route('avisos.destroy', $aviso) }}"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit"
                                    class="text-gray-300 hover:text-red-500 transition-colors text-sm leading-none mt-0.5"
        ...
        {{-- Modal nuevo aviso (Alpine dentro del mismo x-data) --}}
        @role('admin')
        <div x-show="abierto"
            ...
            <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4"
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="abierto = false"
                            class="text-gray-400 hover:text-gray-600 transition-colors text-lg leading-none">✕</button>
                </div>
                ...
                <form method="POST" action="{{ route('avisos.store') }}">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-mono text-gray-500 uppercase tracking-wide mb-1">
                                Título *
                            </label>
                            <input type="text" name="titulo" required
                                   placeholder="Ej: Procedimiento de Red LAN actualizado"
                                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-100">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="url_texto"
                                       placeholder="Ver manual"
                                       class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400">
                            </div>
                        </div>
