...
<head>
<meta charset="UTF-8">
<title>{{ $mantencion->numero_orden }} - Mantencion Preventiva</title>
<style>
...
<img src="{{ public_path('images/logo.png') }}" style="max-height:18mm; max-width:28mm; width:auto; height:auto;">
...
<div class="orden-strip">
    <div class="orden-num">{{ $mantencion->numero_orden }}</div>
    <div class="orden-fecha">Emitido: {{ now()->format('d/m/Y') }} - {{ now()->format('H:i') }} hrs</div>
</div>
...
<div class="seccion">
    ...
    <div class="resumen-cards">
        <div class="resumen-card">
            ...
            <div class="resumen-card-value" style="color:#1a1a1a;">{{ $total }}</div>
            <div class="resumen-card-sub">en esta orden</div>
        </div>
        <div class="resumen-card">
            <div class="resumen-card-label">Operativos</div>
            <div class="resumen-card-value" style="color:#15803d;">{{ $operativo }}</div>
            <div class="resumen-card-sub">{{ $pct_op }}% del total</div>
        </div>
        <div class="resumen-card">
            <div class="resumen-card-label">Con observaciones</div>
            <div class="resumen-card-value" style="color:#92400e;">{{ $obs }}</div>
            <div class="resumen-card-sub">{{ $pct_obs }}% del total</div>
        </div>
        <div class="resumen-card">
            <div class="resumen-card-label">Defectuosos</div>
            <div class="resumen-card-value" style="color:#b91c1c;">{{ $defec }}</div>
            <div class="resumen-card-sub">{{ $pct_def }}% del total</div>
        </div>
    </div>
        ...
        <div class="resumen-body">
                ...
                <table class="resumen-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $t_total }}</td>
                            <td class="{{ $t_op > 0 ? 'num-op' : '' }}">{{ $t_op }}</td>
                            <td class="{{ $t_obs > 0 ? 'num-obs' : '' }}">{{ $t_obs }}</td>
                            <td class="{{ $t_def > 0 ? 'num-def' : '' }}">{{ $t_def }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            <td>Total</td>
                            <td>{{ $total }}</td>
                            <td class="num-op">{{ $operativo }}</td>
                            <td class="num-obs">{{ $obs }}</td>
                            <td class="num-def">{{ $defec }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="resumen-grafico-col">
                <div class="grafico-wrap">
                    ...
                    <table style="width:100%; border-collapse:collapse; height:{{ $bar_h + 12 }}mm;">
                        <tr style="vertical-align:bottom; height:{{ $bar_h }}mm;">
                            <td style="width:8mm; vertical-align:bottom; border-right:1px solid #e5e7eb; padding-right:1.5mm; text-align:right;">
                                <div style="font-size:5.5pt; color:#9ca3af; margin-bottom:{{ $bar_h - 4 }}mm;">{{ $max_val }}</div>
                            ...
                            <td style="vertical-align:bottom; border-bottom:1px solid #e5e7eb; padding-left:2mm;">
                                <table style="width:90%; margin:0 auto; border-collapse:collapse; height:{{ $bar_h }}mm;">
                                    <tr style="vertical-align:bottom; height:{{ $bar_h }}mm;">
                                        <td style="width:33%; vertical-align:bottom; text-align:center; padding:0 3mm;">
                                            @if($h_op > 0)
                                            <div style="background:#16a34a; height:{{ $h_op }}mm; border-radius:3px 3px 0 0;"></div>
                                            @endif
                                        </td>
                                        <td style="width:33%; vertical-align:bottom; text-align:center; padding:0 3mm;">
                                            @if($h_obs > 0)
                                            <div style="background:#d97706; height:{{ $h_obs }}mm; border-radius:3px 3px 0 0;"></div>
                                            @endif
                                        </td>
                                        <td style="width:33%; vertical-align:bottom; text-align:center; padding:0 3mm;">
                                            @if($h_def > 0)
                                            <div style="background:#dc2626; height:{{ $h_def }}mm; border-radius:3px 3px 0 0;"></div>
                                            @endif
                                        </td>
                                    </tr>
                                </table>
                                ...
                                <table style="width:90%; margin:0 auto; border-collapse:collapse;">
                                    <tr>
                                        <td style="width:33%; text-align:center; padding-top:1.5mm;">
                                            <div style="font-size:7pt; font-weight:700; color:#16a34a;">{{ $operativo }}</div>
                                            <div style="font-size:5.5pt; color:#6b7280;">Op.</div>
                                        </td>
                                        <td style="width:33%; text-align:center; padding-top:1.5mm;">
                                            <div style="font-size:7pt; font-weight:700; color:#d97706;">{{ $obs }}</div>
                                            <div style="font-size:5.5pt; color:#6b7280;">Obs.</div>
                                        </td>
                                        <td style="width:33%; text-align:center; padding-top:1.5mm;">
                                            <div style="font-size:7pt; font-weight:700; color:#dc2626;">{{ $defec }}</div>
...
<div style="page-break-inside: avoid;">
        ...
        <table style="width:100%; border-collapse:collapse;">
            <tr>
                <td style="width:50%; padding:2.5mm 3mm; border-right:1px solid #e5e7eb; border-bottom:1px solid #e5e7eb;">
                    ...
                    <div style="font-size:9pt; font-weight:600; color:#111827;">{{ $mantencion->firma_nombre ?? '-' }}</div>
                </td>
                <td style="width:50%; padding:2.5mm 3mm; border-bottom:1px solid #e5e7eb;">
                    ...
                    <div style="font-size:9pt; font-weight:600; color:#111827;">{{ $mantencion->firma_cargo ?? '-' }}</div>
                </td>
            </tr>
            <tr>
    ...
    <div class="doc-footer">
        <div class="footer-left">{{ $mantencion->numero_orden }}</div>
...
<div class="equipo-block">
    <div class="equipo-header clearfix">
        <div class="equipo-header-left">
            <span class="equipo-num">{{ $loop->iteration }}</span>
            <span class="equipo-tipo-text">{{ $tipo_label }}</span>
        </div>
        <div class="equipo-header-right">
            <span class="estado-pill {{ $estado_info['class'] }}">{{ $estado_info['label'] }}</span>
        </div>
    </div>
    ...
    <div class="equipo-datos">
        ...
        <div class="equipo-dato">
            ...
            <div class="equipo-dato-value">{{ $estado_info['label'] }}</div>
        </div>
    </div>

    @foreach($secciones as $seccion => $items)
    <div class="checklist-seccion-title">{{ $seccion }}</div>
    @foreach($items as $respuesta)
    @php
        $resp_labels = [
    ...
    <div class="equipo-obs">
        ...
        <div class="equipo-obs-text">{{ $equipo->observaciones }}</div>
    </div>
    @endif
...
<div class="doc-footer">
    <div class="footer-left">{{ $mantencion->numero_orden }} — Equipo {{ $loop->iteration }} de {{ $total }}</div>
    <div class="footer-right">Generado: {{ now()->format('d/m/Y H:i') }} hrs</div>
</div>
