...
<div class="max-w-2xl">
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <form method="POST" action="{{ route('clientes.update', $cliente) }}" class="p-6">
            ...
            {{-- ══════════════════════════════════════ --}}
            @php
                $sla_activo = old('tiene_sla', $cliente->tiene_sla) ? true : false;
            @endphp
            ...
            <div class="border-t border-gray-100 pt-5 mb-4">
                ...
                <div id="sla_campos" class="{{ $sla_activo ? '' : 'hidden' }}">
                    <div class="bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 mb-4 text-xs text-blue-600">
                        ℹ️ Los tiempos se expresan en <strong>horas</strong>. Si el cliente no cuenta con alguno de los compromisos, ingresa <strong>0</strong>.
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-mono text-gray-500 uppercase tracking-wider mb-1">
                                Tiempo respuesta
                                <span class="text-gray-400 normal-case font-sans">(hrs)</span>
                            </label>
                            <input type="number" name="sla_tiempo_respuesta"
                                   value="{{ old('sla_tiempo_respuesta', $cliente->sla_tiempo_respuesta ?? 0) }}"
                                   min="0" placeholder="0"
                                   class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition-all">
                        ...
                        <div>
                            ...
                            <input type="number" name="sla_tiempo_solucion"
                                   value="{{ old('sla_tiempo_solucion', $cliente->sla_tiempo_solucion ?? 0) }}"
                                   min="0" placeholder="0"
                                   class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition-all">
                            <p class="text-xs text-gray-400 mt-1">Tiempo máximo para resolver</p>
                        </div>

                        {{-- Tiempo cambio equipo --}}
                        <div>
                            <label class="block text-xs font-mono text-gray-500 uppercase tracking-wider mb-1">
                                Cambio de equipo
                                <span class="text-gray-400 normal-case font-sans">(hrs)</span>
                            </label>
                            <input type="number" name="sla_tiempo_cambio_equipo"
                                   value="{{ old('sla_tiempo_cambio_equipo', $cliente->sla_tiempo_cambio_equipo ?? 0) }}"
                                   min="0" placeholder="0"
                                   class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition-all">
